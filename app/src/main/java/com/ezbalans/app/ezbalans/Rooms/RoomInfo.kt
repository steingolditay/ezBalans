...
class RoomInfo : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (bundle != null) {
            val roomUID = bundle.getString(Constants.room_uid)!!
...
//                        admin = true
        ...
        for (entry in users) {
            ...
            if (room.residents[user.uid] == Constants.added) {
                ...
                if (admins.contains(user.uid)) {
                    rowView = LayoutInflater.from(this).inflate(R.layout.row_user, binding.adminsContainer, false)
                ...
                // user is regular
                    ...
                    rowView = LayoutInflater.from(this).inflate(R.layout.row_user, binding.residentsContainer, false)
                    val layoutParams = ConstraintLayout.LayoutParams(ConstraintLayout.LayoutParams.MATCH_PARENT, ConstraintLayout.LayoutParams.WRAP_CONTENT)
                    layoutParams.setMargins(10, 10, 10, 10)
    ...
    private fun loadRoomDetails() {
        if (room.image.isNotEmpty())
            ...
            Picasso.get().load(R.drawable.default_room).into(binding.image)
    ...
    private fun removeResident(user: User, view: View) {
        databaseReference.child(Constants.rooms).child(room.uid).child(Constants.residents).child(user.uid).setValue(Constants.removed).addOnSuccessListener {
